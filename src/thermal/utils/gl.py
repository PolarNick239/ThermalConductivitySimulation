...
from OpenGL.GL.VERSION.GL_1_0 import GLenum, GLuint, glTexImage1D, glTexImage2D, glViewport, glEnable, glDisable, glFinish, glReadPixels, glGetTexImage
...
from thermal.utils.gl_context import GLContext, GLFWContext
...
class GLExecutor(AsyncExecutor):
    ...
    def init_gl_context(self):
        if not self._initialized:
            yield from super(GLExecutor, self).map(self._init_gl_context)
    ...
    def map(self, fn, *args):
        ...
        return (yield from super(GLExecutor, self).map(self._with_activated_context, fn, *args))
...
def create_image_texture(path):
    image = PIL.Image.open(str(path))
    w, h = image.size[0], image.size[1]
    bytes = image.tobytes("raw", "RGBA", 0, -1)
    texture = Texture2D()
    with texture:
        ...
        with configure_pixel_store([(GL_UNPACK_ALIGNMENT, 1)]):
            glTexImage2D(texture.target, 0, GL_RGBA, w, h, 0, GL_BGRA, GL_UNSIGNED_BYTE, bytes)
    return (w, h), texture
